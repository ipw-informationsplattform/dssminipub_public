  ...
  SELECT
    ...
    CASE
      WHEN l.baulicherzustand IN ('Z0','Z1','Z2','Z3','Z4','unbekannt') THEN
        CASE
          WHEN l.funktionhydraulisch IN ('Pumpendruckleitung','Vakuumleitung') THEN 
            CASE
              WHEN l.baulicherzustand = 'Z0' THEN 'L_PD_Z0'
              WHEN l.baulicherzustand = 'Z1' THEN 'L_PD_Z1'
              WHEN l.baulicherzustand = 'Z2' THEN 'L_PD_Z2'
              WHEN l.baulicherzustand = 'Z3' THEN 'L_PD_Z3'
              WHEN l.baulicherzustand = 'Z4' THEN 'L_PD_Z4'
              WHEN l.baulicherzustand = 'unbekannt' THEN 'L_PD_unbekannt'
            END
          ELSE
            CASE 
              WHEN l.nutzungsart_ist IN ('Mischabwasser','Schmutzabwasser','Industrieabwasser','entlastetes_Mischabwasser','andere','unbekannt') THEN
                CASE
                  WHEN l.baulicherzustand = 'Z0' THEN 'L_MA_SA_Entl_Z0'
                  WHEN l.baulicherzustand = 'Z1' THEN 'L_MA_SA_Entl_Z1'
                  WHEN l.baulicherzustand = 'Z2' THEN 'L_MA_SA_Entl_Z2'
                  WHEN l.baulicherzustand = 'Z3' THEN 'L_MA_SA_Entl_Z3'
                  WHEN l.baulicherzustand = 'Z4' THEN 'L_MA_SA_Entl_Z4'
                  WHEN l.baulicherzustand = 'unbekannt' THEN 'L_MA_SA_Entl_unbekannt'
                END
              WHEN l.nutzungsart_ist IN ('Reinabwasser','Niederschlagsabwasser','Bachwasser') THEN
                CASE
