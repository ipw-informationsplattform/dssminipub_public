...
mangel_k AS (
  ...
  LEFT JOIN (
    ...
    FROM se_zpk_pub.paa_schacht_se
      UNION
    SELECT oid_dss, stilid
    FROM se_zpk_pub.paa_schacht_dr 
      UNION
    SELECT oid_dss, stilid
    FROM se_zpk_pub.paa_sbw
      UNION
    SELECT oid_dss, stilid
    FROM se_zpk_pub.saa_schacht_se 
      UNION
    SELECT oid_dss, stilid
    FROM se_zpk_pub.saa_schacht_dr
      UNION
    SELECT oid_dss, stilid
    FROM se_zpk_pub.saa_sbw) u
...
INSERT INTO se_zpk_pub.mangel_knoten (oid_dss, lage, beschreibung, stilid)
  SELECT
    t_ili_tid,
    lage,
