 ...
 * Inhalt:                PAA Schächte SE, Status 'wird aufgehoben'
 * Script:                vsadssmini_paa_schacht_se_wirdaufgehoben.sql
  ...
  SELECT
    ...
    CASE
      WHEN k.funktion IN ('Abflusslose_Toilette','abflussloseGrube','Absturzbauwerk','Abwasserfaulraum','Duekerkammer','Einlaufschacht','Faulgrube', 'Geleiseschacht','Geschiebefang','Guellegrube','Havariebecken','Klaergrube','Kombischacht','Kontroll_Einsteigschacht','Schlammsammler') THEN
        CASE
          WHEN k.nutzungsart_geplant = 'Bachwasser' THEN 'P_BW_aufg'
          WHEN k.finanzierung = 'oeffentlich' THEN
            CASE 
              WHEN k.nutzungsart_geplant = 'entlastetes_Mischabwasser' THEN 'P_Entl_aufg'
              WHEN k.nutzungsart_geplant = 'Mischabwasser' THEN 'P_MA_aufg'
              WHEN k.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'P_SA_aufg'
              WHEN k.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'P_RA_aufg'
              WHEN k.nutzungsart_geplant IN ('andere','unbekannt') THEN 'P_unbekannt_aufg'
            END
          WHEN k.finanzierung != 'oeffentlich' AND eig.organisationstyp = 'Privat' THEN
            CASE
              WHEN k.nutzungsart_geplant = 'Mischabwasser' THEN 'P_MA_LE_aufg'
              WHEN k.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'P_SA_LE_aufg'
              WHEN k.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'P_RA_LE_aufg'
              WHEN k.nutzungsart_geplant IN ('andere','unbekannt','entlastetes_Mischabwasser') THEN 'P_unbekannt_LE_aufg'
            END
          WHEN k.finanzierung != 'oeffentlich' AND eig.organisationstyp != 'Privat' THEN
            CASE
              WHEN k.nutzungsart_geplant = 'Mischabwasser' THEN 'P_MA_dr_aufg'
              WHEN k.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'P_SA_dr_aufg'
              WHEN k.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'P_RA_dr_aufg'
              WHEN k.nutzungsart_geplant IN ('andere','unbekannt','entlastetes_Mischabwasser') THEN 'P_unbekannt_dr_aufg'
            END
        END
    END AS stilid
...
INSERT INTO se_mp_pub.paa_schacht_se (baujahr, baulicherzustand, betreiber_bezeichnung, betreiber_organisationstyp, bezeichnung, deckelkote, eigentuemer_bezeichnung, eigentuemer_organisationstyp, finanzierung, funktion, funktionhierarchisch, lage, nutzungsart_ist, nutzungsart_geplant, oid_dss, sohlenkote, astatus, stilid, zustandserhebung_jahr)
  SELECT
    baujahr,
    baulicherzustand,
