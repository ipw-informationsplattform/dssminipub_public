 ...
 * Inhalt:                PAA Sonderbauwerke, Status 'projektiert'
 * Script:                vsadssmini_paa_sbw_projektiert.sql
...
sbw AS (
  ...
  SELECT
    ...
    CASE
      WHEN k.funktion = 'Versickerungsanlage' THEN 'P_VS_proj'
      WHEN k.funktion = 'Messstelle' THEN 'P_MST_proj'
      WHEN k.funktion LIKE 'Einleitstelle%' THEN 'P_EST_proj'
      WHEN k.funktion = 'Trennbauwerk' THEN 'P_TB_proj'
      WHEN k.funktion = 'Duekeroberhaupt' THEN 'P_DU_proj'
      WHEN k.funktion = 'Pumpwerk'  THEN 'P_PW_proj'
      WHEN k.funktion IN ('Regenbecken_Regenrueckhaltebecken','Regenbecken_Regenrueckhaltekanal') THEN 'P_RRB_proj'
      WHEN k.funktion LIKE 'Regenbecken_%' AND k.funktion NOT IN ('Regenbecken_Regenrueckhaltebecken','Regenbecken_Regenrueckhaltekanal') THEN 'P_RB_proj'
      WHEN k.funktion = 'Regenueberlauf' THEN 'P_RU_proj'
      WHEN k.funktion IN ('KLARA','ARABauwerk') THEN 'P_ARA_proj'
      WHEN (k.funktion IN ('Behandlungsanlage','Wirbelfallschacht') OR k.funktion LIKE '%abscheider')  THEN 'P_uSBW_proj'
      WHEN k.funktion IN ('andere','unbekannt') AND sk.paa_knotenref IS NOT NULL THEN 'P_uSBW_proj'
    END AS stilid
...
INSERT INTO se_mp_pub.paa_sbw (baujahr, baulicherzustand, betreiber_bezeichnung, betreiber_organisationstyp, bezeichnung, eigentuemer_bezeichnung, eigentuemer_organisationstyp, funktion, funktionhierarchisch, lage, oid_dss, astatus, stilid, zustandserhebung_jahr)
  SELECT
    baujahr,
    baulicherzustand,
