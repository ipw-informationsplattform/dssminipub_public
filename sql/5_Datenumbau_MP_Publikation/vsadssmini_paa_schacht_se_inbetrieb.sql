 ...
 * Inhalt:                PAA Schächte SE, Status 'in Betrieb'
 * Script:                vsadssmini_paa_schacht_se_inbetrieb.sql
  ...
  SELECT
    ...
    CASE
      WHEN k.funktion IN ('Abflusslose_Toilette','abflussloseGrube','Absturzbauwerk','Abwasserfaulraum','Duekerkammer','Einlaufschacht','Faulgrube', 'Geleiseschacht','Geschiebefang','Guellegrube','Havariebecken','Klaergrube','Kombischacht','Kontroll_Einsteigschacht','Schlammsammler') THEN
        CASE
          WHEN k.nutzungsart_geplant = 'Bachwasser' THEN 'P_BW'
          WHEN k.finanzierung = 'oeffentlich' THEN
            CASE 
              WHEN k.nutzungsart_geplant = 'entlastetes_Mischabwasser' THEN 'P_Entl'
              WHEN k.nutzungsart_geplant = 'Mischabwasser' THEN 'P_MA'
              WHEN k.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'P_SA'
              WHEN k.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'P_RA'
              WHEN k.nutzungsart_geplant IN ('andere','unbekannt') THEN 'P_unbekannt'
            END
          WHEN k.finanzierung != 'oeffentlich' AND eig.organisationstyp = 'Privat' THEN
            CASE
              WHEN k.nutzungsart_geplant = 'Mischabwasser' THEN 'P_MA_LE'
              WHEN k.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'P_SA_LE'
              WHEN k.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'P_RA_LE'
              WHEN k.nutzungsart_geplant IN ('andere','unbekannt','entlastetes_Mischabwasser') THEN 'P_unbekannt_LE'
            END
          WHEN k.finanzierung != 'oeffentlich' AND eig.organisationstyp != 'Privat' THEN
            CASE
              WHEN k.nutzungsart_geplant = 'Mischabwasser' THEN 'P_MA_dr'
              WHEN k.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'P_SA_dr'
              WHEN k.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'P_RA_dr'
              WHEN k.nutzungsart_geplant IN ('andere','unbekannt','entlastetes_Mischabwasser') THEN 'P_unbekannt_dr'
            END
        END
    END AS stilid
