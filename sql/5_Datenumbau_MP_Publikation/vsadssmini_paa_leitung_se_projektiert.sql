  ...
  SELECT
    ...
    CASE
      ...
      WHEN eig.organisationstyp = 'Privat' AND l.finanzierung != 'oeffentlich' AND l.funktionhierarchisch IN ('PAA.Sammelkanal','PAA.Liegenschaftsentwaesserung') THEN
        CASE 
          WHEN l.funktionhydraulisch IN ('Pumpendruckleitung','Vakuumleitung') THEN 'L_Gem_PD_proj'
          WHEN l.nutzungsart_geplant = 'entlastetes_Mischabwasser' THEN 'L_Gem_Entl_proj'
          WHEN l.nutzungsart_geplant = 'Mischabwasser' THEN 'L_Gem_MA_proj'
          WHEN l.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'L_Gem_SA_proj'
          WHEN l.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'L_Gem_RA_proj'
          WHEN l.nutzungsart_geplant IN ('andere','unbekannt') THEN 'L_Gem_unbekannt_proj'
        END
