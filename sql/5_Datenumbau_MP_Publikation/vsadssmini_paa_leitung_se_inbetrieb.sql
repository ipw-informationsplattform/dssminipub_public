 ...
 * Inhalt:                PAA Leitungen SE, Status 'in Betrieb'
 * Script:                vsadssmini_paa_leitung_se_inbetrieb.sql
...
leitungen AS (
  ...
  SELECT
    ...
    CASE
      WHEN l.nutzungsart_geplant = 'Bachwasser' AND profiltyp != 'offenes_Profil' THEN 'L_eindol'
      WHEN eig.organisationstyp = 'Abwasserverband' AND funktionhierarchisch LIKE 'PAA.%' AND profiltyp != 'offenes_Profil' THEN 
        CASE 
          ...
          WHEN l.nutzungsart_geplant = 'entlastetes_Mischabwasser' THEN 'L_VK_Entl'
          WHEN l.nutzungsart_geplant = 'Mischabwasser' THEN 'L_VK_MA'
          WHEN l.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'L_VK_SA'
          WHEN l.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'L_VK_RA'
          WHEN l.nutzungsart_geplant IN ('andere','unbekannt') THEN 'L_VK_unbekannt'
        END
      WHEN eig.organisationstyp = 'Privat' AND l.finanzierung != 'oeffentlich' AND l.funktionhierarchisch IN ('PAA.Sammelkanal','PAA.Liegenschaftsentwaesserung') THEN
        CASE 
          WHEN l.funktionhydraulisch IN ('Pumpendruckleitung','Vakuumleitung') THEN 'L_Gem_PD'
          WHEN l.nutzungsart_geplant = 'entlastetes_Mischabwasser' THEN 'L_Gem_Entl'
          WHEN l.nutzungsart_geplant = 'Mischabwasser' THEN 'L_Gem_MA'
          WHEN l.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'L_Gem_SA'
          WHEN l.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'L_Gem_RA'
          WHEN l.nutzungsart_geplant IN ('andere','unbekannt') THEN 'L_Gem_unbekannt'
        END
      ...
      ELSE
        CASE 
          WHEN l.finanzierung = 'oeffentlich' THEN
            CASE 
              WHEN l.nutzungsart_geplant = 'entlastetes_Mischabwasser' THEN 'L_Entl'
              WHEN l.nutzungsart_geplant = 'Mischabwasser' THEN 'L_MA'
              WHEN l.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'L_SA'
              WHEN l.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'L_RA'
              WHEN l.nutzungsart_geplant IN ('andere','unbekannt') THEN 'L_unbekannt'
            END
          WHEN l.finanzierung != 'oeffentlich' AND eig.organisationstyp = 'Privat' THEN
            CASE
              WHEN l.nutzungsart_geplant = 'Mischabwasser' THEN 'L_MA_LE'
              WHEN l.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'L_SA_LE'
              WHEN l.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'L_RA_LE'
              WHEN l.nutzungsart_geplant IN ('andere','unbekannt','entlastetes_Mischabwasser') THEN 'L_unbekannt_LE'
            END
          WHEN l.finanzierung != 'oeffentlich' AND eig.organisationstyp != 'Privat' THEN
            CASE
              WHEN l.nutzungsart_geplant = 'Mischabwasser' THEN 'L_MA_dr'
              WHEN l.nutzungsart_geplant IN ('Schmutzabwasser','Industrieabwasser') THEN 'L_SA_dr'
              WHEN l.nutzungsart_geplant IN ('Reinabwasser','Niederschlagsabwasser') THEN 'L_RA_dr'
              WHEN l.nutzungsart_geplant IN ('andere','unbekannt','entlastetes_Mischabwasser') THEN 'L_unbekannt_dr'
            END
        END
    END AS stilid
  FROM
